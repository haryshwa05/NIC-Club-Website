...
import workshopsData from './WorkshopsData.json'; // Import the JSON file
import { Button } from "@/components/ui/button";
import { Laptop } from 'lucide-react'; // Import the Laptop icon from lucide-react
                ...
                <div className="bg-background rounded-lg text-black overflow-hidden shadow-lg transition-all hover:shadow-xl relative cursor-pointer" style={{ height: "100%", width: "100%" }}>
                  ...
                  <div className="relative w-full h-full" style={{ aspectRatio: "3/4" }}>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4 pt-6 bg-white">
                      <div>
                        <h3 className="text-md md:text-lg font-bold akira-expanded">{workshop.title}</h3>
                        <p className="text-muted-foreground text-sm poppins-regular">{workshop.description}</p>
                      </div>
                ...
                >
                  ...
                  <div className="flex flex-col justify-center items-start h-full space-y-4 p-4 border-2 border-red-600 rounded-lg w-full box-border">
                    ...
                    <div className="flex items-center space-x-2">
                      <h3 className="text-md font-bold akira-expanded">{workshop.title}</h3>
                    </div>
                    <p className="text-xs md:text-sm poppins-regular">{workshop.backDetails}</p> {/* Show back details */}
                  </div>
                </div>
              </ReactCardFlip>
            </div>
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex flex-col justify-center items-start h-full space-y-4 p-4 border-2 border-red-600 rounded-lg w-full box-border">
                      {/* Laptop icon and title */}
                      <div className="flex items-center space-x-2">
                        <Laptop className="w-5 h-5" /> {/* Laptop icon from lucide-react */}
                        <h3 className="text-xl font-bold">{workshop.title}</h3>
                      </div>
